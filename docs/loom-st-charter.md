...
```
ST JSON 文件 ──[Source Passes]──▶ Fragment[] ──[Compile Passes]──▶ Fragment[] ──[Emit Pass]──▶ messages[]
```
...
输出：
- 一份合理的 messages[]（OpenAI 格式），能直接喂给 `streamText()`
...
下表是 M0 的初始拆分方案。**列出 reads/writes 是强制要求**，因为 `loom-observability.md` 的冲突检测依赖于它。如果某个 Pass 的 writes 声明不住，说明它的职责划分错了，应该拆成多个 Pass。

| # | 阶段 | Pass 名 | reads | writes | requires | provides |
|---|---|---|---|---|---|---|
| 1 | Source | `LoadCharacterCard` | — | * | — | `character-loaded` |
| 2 | Source | `LoadPreset` | — | * | — | `preset-loaded` |
| 3 | Source | `LoadWorldInfo` | — | * | — | `worldinfo-loaded` |
| 4 | Source | `LoadChatHistory` | — | * | — | `chat-loaded` |
...
| 多模型适配（Claude / Gemini messages 格式） | Emit Pass 先只做 OpenAI 格式 |
| 性能测试 | 没跑通之前谈性能是自我欺骗 |
